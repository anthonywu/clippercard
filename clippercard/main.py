...
def _get_client_auth(args):
    ...
    if not (username and password):
        ...
        if not os.path.exists(config_file_path):
            raise ClipperCardCommandError('Login config file {0} does not exist.'.format(
                config_file_path
                ))
        try:
            ...
            raise ClipperCardCommandError('Account config section {0} is not found in {1}'.format(
                repr(args['--account']),
                config_file_path
                ))
