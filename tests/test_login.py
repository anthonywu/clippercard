...
def make_cookie(name, value, domain="", path="/"):
    """Construct a stdlib Cookie with the defaults the session expects."""
    return Cookie(
        version=0,
        name=name,
        value=value,
...
def test_login_reuses_saved_cookies_when_dashboard_loads(tmp_path):
    ...
    seed_session.cookies.jar.set_cookie(
        make_cookie("JSESSIONID", "saved-session", domain="www.clippercard.com", path="/")
    )
...
def test_login_saves_cookie_jar_after_successful_login(tmp_path):
    ...
    def fake_post(self, url, data=None, headers=None):
        self.cookies.jar.set_cookie(make_cookie("JSESSIONID", "fresh-session", domain="www.clippercard.com", path="/"))
        return DummyResponse(dashboard_html, url)
...
def test_keychain_cookie_store_saves_and_loads_cookies():
    ...
    session.cookies.jar.set_cookie(make_cookie("JSESSIONID", "fresh-session", domain="www.clippercard.com", path="/"))
    ...
    assert reused_session.cookies.jar._cookies["www.clippercard.com"]["/"]["JSESSIONID"].value == "fresh-session"
...
def test_keychain_cookie_store_migrates_existing_file_cookies(tmp_path):
    ...
    assert session.cookies.jar._cookies["www.clippercard.com"]["/"]["JSESSIONID"].value == "saved-session"
    assert cookie_jar_path.exists()
